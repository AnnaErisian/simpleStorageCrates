...
public class GuiHandlerMBE30 implements IGuiHandler {
	...
	public Object getServerGuiElement(int ID, EntityPlayer player, World world, int x, int y, int z) {
		if (ID < getGuiID()+1 || ID > getGuiID()+4) {
			System.err.println("Invalid ID: expected " + getGuiID() + "+1 to +4, received " + ID);
		}
		...
		if (tileEntity instanceof TileEntityInventoryBasic) {
			...
			return new ContainerBasic(player.inventory, tileEntityInventoryBasic, ID - getGuiID());
		}
		return null;
	}
	...
	public Object getClientGuiElement(int ID, EntityPlayer player, World world, int x, int y, int z) {
		...
		if (tileEntity instanceof TileEntityInventoryBasic) {
			TileEntityInventoryBasic tileEntityInventoryBasic = (TileEntityInventoryBasic) tileEntity;
			return new GuiInventoryBasic(player.inventory, tileEntityInventoryBasic, ID - getGuiID());
		}
		return null;
	}
}
